  ...
  (let ()
    (define (test* expect fun args kws kvs)
      ...
      (let ([res (if (procedure? fun)
       (let ([gambit-fun (gambit-lookup fun)])
         (if gambit-fun
           (to-rktnum (if kws (keyword-apply fun kws kvs (map from-rktnum args))
            (apply gambit-fun (map from-rktnum args))))
